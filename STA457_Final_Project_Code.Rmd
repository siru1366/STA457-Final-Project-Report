...
```{r}
# Load and process Cocoa Price Data
...
# Clean and sort cocoa price data, remove invalid values
cocoa_prices <- cocoa_prices %>%
  select(Date, Price) %>%
  arrange(Date) %>%
  filter(!is.na(Price) & Price > 0)

# Compute log and differenced log price (aligned with 457_project)
cocoa_prices <- cocoa_prices %>%
  mutate(
    log_price = log(Price),
...
# Create complete sequence of daily dates
...
```{r}
desc_table <- describe(cocoa_data %>% select(Price, log_price, diff_log_price, PRCP, TAVG, TMAX, TMIN))
print(desc_table)

# Histogram of raw cocoa price
ggplot(cocoa_data, aes(x = Price)) +
...
### ARIMA model
```{r}
cocoa_prices <- read.csv("/Users/tansirui/Downloads/Daily Prices_ICCO.csv", stringsAsFactors = FALSE)
climate_data <- read.csv("/Users/tansirui/Downloads/Ghana_data.csv", stringsAsFactors = FALSE)
...
colnames(cocoa_prices)[colnames(cocoa_prices) == "Date"] <- "DATE"


cocoa_prices$ICCO.daily.price..US..tonne. <- gsub(",", "", cocoa_prices$ICCO.daily.price..US..tonne.)  # Remove commas
cocoa_prices$ICCO.daily.price..US..tonne. <- as.numeric(cocoa_prices$ICCO.daily.price..US..tonne.)  # Convert to numeric


colnames(cocoa_prices)[colnames(cocoa_prices) == "ICCO.daily.price..US..tonne."] <- "CocoaPrice"
cocoa_prices_month <-cocoa_prices%>%
  mutate(YearMonth = floor_date(DATE, "month")) %>%
  group_by(YearMonth) %>%
  summarise(CocoaPrice = mean(CocoaPrice, na.rm = TRUE))
ts_data<- ts(cocoa_prices$CocoaPrice)
...
# Extract model-estimated volatility
volatility <- sigma(garch_fit)
vol_df <- tibble(
  Date = cocoa_data$Date[-1],  # remove first date (due to diff)
  Volatility = as.numeric(volatility)
)

# Compute realized (actual) volatility
actual_return <- abs(log_returns)
realized_vol <- rollapply(actual_return, width = 5, FUN = sd, align = "right", fill = NA)

# Align dates
realized_vol_df <- tibble(
  Date = cocoa_data$Date[-1][-c(1:4)],  # drop first 4 NAs after rolling
  RealizedVol = realized_vol[-(1:4)]
)

# volatilities
combined_vol <- vol_df %>%
  inner_join(realized_vol_df, by = "Date")

# Plot
ggplot(combined_vol) +
  geom_line(aes(x = Date, y = Volatility, color = "GARCH Volatility"), linewidth = 0.8) +
  geom_line(aes(x = Date, y = RealizedVol, color = "Realized Volatility"), linewidth = 0.8) +
  scale_color_manual(values = c("GARCH Volatility" = "red", "Realized Volatility" = "yellow")) +
  labs(
    title = "GARCH Estimated vs Realized Volatility of Cocoa Prices",
    x = "Date",
    y = "Volatility",
...
# Check Stationarity
alpha1 <- garch_fit@fit$coef["alpha1"]
beta1 <- garch_fit@fit$coef["beta1"]
sum_ab <- alpha1 + beta1
...
create_lags <- function(data, lags = 1:5) {
  set.seed(1009037221)
  for (lag in lags) {
...
# Only use last 150 rows for tuning
cocoa_data_short <- cocoa_data[(nrow(cocoa_data) - 150):nrow(cocoa_data), ]

# Repetition count for walk-forward
N_rep <- 5
...
expanding_window_cv <- function(data, model_type = "rf", lag = 5, step = 7) {
  ...
  if (nrow(data) < 30) {
    stop("Not enough data for expanding window training.")
  }
...
```{r}
future_rf <- predict_next_days_rf(
...
plot_df <- bind_rows(
  ...
  future_rf %>% select(Date, Price) %>% mutate(Type = "Forecasted")
)
