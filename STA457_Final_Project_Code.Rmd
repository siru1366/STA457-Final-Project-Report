...
### Merge
```{r}
cocoa_data <- left_join(ghana_weather, cocoa_prices, by = c("Date" = "DATE")) %>% mutate(log_price = log(Price),
         diff_log_price = c(NA, diff(log_price))) %>% drop_na()
```
...
garch_vol <- tibble(
  Date = returns$Date,
  GARCH_Vol = as.numeric(sigma(fit_garch))
)

realized_vol <- rollapply(abs(returns$log_return), width = 5, FUN = sd, align = "right", fill = NA)
...
vol_plot_data <- garch_vol %>%
  inner_join(realized_df, by = "Date")

vol_plot_data <- garch_vol %>%
  inner_join(realized_df, by = "Date")

ggplot(vol_plot_data, aes(x = Date)) +
  geom_line(aes(y = GARCH_Vol, color = "GARCH Estimated"), linewidth = 0.8) +
  geom_line(aes(y = Realized_Vol, color = "Realized (Rolling SD)"), linewidth = 0.8) +
  scale_color_manual(values = c("GARCH Estimated" = "red", "Realized (Rolling SD)" = "orange")) +
  labs(
    title = "GARCH Estimated vs Realized Volatility",
    x = "Date",
    y = "Volatility",
    color = "Type"
  ) +
